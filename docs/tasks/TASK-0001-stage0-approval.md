...
- BLOCKED: the `Dependency and licence review` check on pull request `#1` reports that Dependency Review is unsupported because Dependency Graph is not enabled. The owner reported enabling it and two failed-job reruns were executed, but the check retained the same failure and the authenticated dependency-graph SBOM endpoint returned `404 Not Found`.
